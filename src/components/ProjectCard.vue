<template>
    <div class="project__product" :id="card.idProject">
        <img class="project__productPhoto" :src="require(`@/assets/imgs/projectPage/${imagePath}/${card.image}`)"
            alt="kitchen's photo">
        <div class="project__star">
            <svg xmlns="http://www.w3.org/2000/svg" width="36" height="35" viewBox="0 0 36 35" fill="none"
                @click="changeIsStarred({ chapter: imagePath, id: id })" :class="classStar">
        ...
        <div class="project__details">
            <div class="project__productText">
                ...
                <p class="project__productBreadcrumbs">
                    <RouterLink :to="breadcrumb.linkTo" v-for="breadcrumb in card.breadcrumbs" :key="breadcrumb.id"
                        @click="changeTitleCurrentArticle(card.title)" class="project__productBreadcrumb">
                        {{ breadcrumb.linkText }}
                    </RouterLink>
                </p>
            </div>
            <RouterLink 
            to="/projectDetails"
            @click="changeTitleCurrentArticle(card.title)"
            class="project__link">
...
export default {
    ...
    data() {
        return {
            id: this.card.idProject
        }
    },
    methods: {
        ...mapMutations(['changeIsStarred', 'changeTitleCurrentArticle']),
